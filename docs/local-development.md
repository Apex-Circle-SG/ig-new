...
For production-mode verification run `SITE_INDEXABLE=true NEXT_PUBLIC_ADS_ENABLED=true npm run build` then `SITE_INDEXABLE=true NEXT_PUBLIC_ADS_ENABLED=true npm run start`. Browser checks validate this launch configuration; unit tests also validate non-indexable previews. The isolated calculator document is generated automatically before dev/build/typecheck. Playwright starts a production server automatically if one is not already present. `npx playwright install chromium` installs the browser; minimal Linux environments may need `npx playwright install --with-deps chromium`.
