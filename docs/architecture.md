...
`/ask/` uses approved-public-content retrieval and narrow explicit deterministic income/drawdown calculations. The server validates bounded questions, origins, security tokens and rate limits; it never forwards raw questions to Datadog. A bounded server-only Datadog workflow selects approved public excerpt IDs from a finite topic/source packet; the app reconstructs source text locally. The browser keeps conversation state in memory. `/admin/` uses a generated server secret and exposes operational aggregates and read-only quality checks; unfinished migration gates cannot be approved there.
