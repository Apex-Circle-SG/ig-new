...
`/ask/` uses approved-public-content retrieval and narrow explicit deterministic income/drawdown calculations. The server validates bounded questions, origins, security tokens and rate limits. A bounded server-only Datadog workflow selects approved public excerpt IDs from a finite topic/source packet; the app reconstructs source text locally. An optional general-answer path sends a minimized question after explicit owner authorization for the configured agent; a tool-free attestation is supported separately. General prompts and answers are never persisted or cached by the application; provider retention still applies. The browser keeps conversation state in memory. `/admin/` uses a generated server secret and exposes operational aggregates and read-only quality checks; unfinished migration gates cannot be approved there.

Optional analytics uses explicit consent and enumerated events only. Essential operation counters are separate daily files, both retained30days. A standalone Datadog metrics worker can export thirteen fixed gauges. Private Bits workflows use an independent allowlist, timeout and persistent budget ledger and remain disabled pending account review. No raw prompts, financial values or PII are telemetry. See [the implementation contract](implementation-2026-09-13.md), [content operations](../scripts/content-sync/README.md), [Ask](genie.md), and [observability](observability.md).
