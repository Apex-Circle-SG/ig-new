...
| DD-01 | MANUAL_REQUIRED | Credentials and agent/workflow IDs now verified; one API-triggered synthetic test succeeded with INSIGHTGINIE_READY. Remaining production work: reviewed agent tools/instructions, strict structured output, rate limits and a restricted service-account identity with retested execution permissions. | Production operational integration; authentication and basic agent execution work |
| DD-02 | EXTERNAL_SERVICE_REQUIRED | Available AI Credits, recurring spend ceiling, model configuration and account-specific retention/redaction controls. API execution/result retrieval passed one synthetic test; no credit balance or production allowance has been established. | Recurring production execution and capacity planning |
...
| DD-04 | MANUAL_REQUIRED | Confirm monitoring entitlements and the interpretation of a public RUM client token versus secret API/application keys. Keys are present in the ignored server .env; production SDKs/runtime integration remain unconfigured. | Browser RUM configuration |
